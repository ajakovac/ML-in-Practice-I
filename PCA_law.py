...
class PCA_law:
    ...
    def classify(self, X):
        return np.linalg.norm((X-self.mu)@self.eigenvectors[:, self.n_components:], axis=1)
...
class PCA_classifier:
    ...
    def predict(self, X):
        return np.array([self.yclasses[k] for k in np.argmin(np.array([self.laws[i].classify(X) for i in range(len(self.yclasses))]), axis=0)])
